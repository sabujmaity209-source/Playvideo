...
function openDB(){
  return new Promise((res, rej)=>{
    const r = indexedDB.open("PlayVideoDB",1)
...
async function saveToDB(video){
  ...
  db.transaction("videos","readwrite").objectStore("videos").put(video)
}
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      const withUrl = all.map(v=> ({...v, video_url: URL.createObjectURL(v.blob)}))
      setVideos(withUrl)
      setLoading(false)
    })()
  },[])
  ...
  async function upload(){
    ...
    const newV = {id: Date.now(), title, blob: file, views: Math.floor(Math.random()*900)+10, created_at: new Date().toISOString()}
    await saveToDB(newV)
    const withUrl = {...newV, video_url: URL.createObjectURL(file)}
    setVideos([withUrl,...videos])
    setTitle(""); setFile(null); setShowUp(false)
    alert("PERMANENT SAVE HOYE GECHE BOSS! 🚀 Refresh korleo thakbe!")
  }

  function openVideo(v){ setSelected(v); setPage("Watch"); window.scrollTo(0,0); setVideos(prev=> prev.map(x=> x.id===v.id? {...x, views:x.views+1}:x)) }
  ...
  return (
    <div style={{background:'#0f0f0f', color:'white', minHeight:'100vh', paddingBottom:80}}>
      ...
      {showUp && <div style={{background:'#212121', margin:12, padding:15, borderRadius:12}}><input placeholder="Title..." value={title} onChange={e=>setTitle(e.target.value)} style={{width:'100%', padding:12, borderRadius:8, border:0, background:'#333', color:'white', marginBottom:10, boxSizing:'border-box'}}/><input type="file" accept="video/*" onChange={e=>setFile(e.target.files[0])} style={{width:'100%', marginBottom:10}}/><button onClick={upload} style={{width:'100%', padding:12, background:'red', color:'white', border:0, borderRadius:8, fontWeight:'bold'}}>PUBLISH PERMANENT 🚀</button></div>}
      {selected && page==="Watch"? <div><video src={selected.video_url} controls autoPlay style={{width:'100%', maxHeight:'55vh', background:'black'}}/><div style={{padding:12}}><h3>{selected.title}</h3><p style={{color:'#aaa'}}>{selected.views} views</p><button onClick={()=>{setSelected(null); setPage("Home")}} style={{padding:'8px 14px', borderRadius:20, border:0}}>← Back</button></div></div> : page==="Home" && <div>{filtered.length===0? <p style={{textAlign:'center', color:'#aaa', marginTop:60}}>Kono video nei - Upload korun Boss!</p> : filtered.map(v=><div key={v.id} onClick={()=>openVideo(v)} style={{marginBottom:10, position:'relative'}}><video src={v.video_url} style={{width:'100%', aspectRatio:'16/9', background:'black'}} muted/><button onClick={(e)=>delVideo(v.id,e)} style={{position:'absolute', top:8, right:8, background:'red', color:'white', border:0, padding:'5px 8px', borderRadius:6, fontSize:12}}>Delete</button><div style={{padding:'8px 12px'}}><b>{v.title}</b><br/><small style={{color:'#aaa'}}>{v.views} views • Permanent</small></div></div>)}</div>}
      <div style={{position:'fixed', bottom:0, left:0, right:0, background:'#0f0f0f', display:'flex', justifyContent:'space-around', padding:'8px 0', borderTop:'1px solid #222'}}><div onClick={()=>{setPage("Home"); setSelected(null)}} style={{textAlign:'center', color:'white', fontSize:11}}><div style={{fontSize:18}}>🏠</div>Home</div><div style={{textAlign:'center', color:'#aaa', fontSize:11}}><div style={{fontSize:18}}>🎬</div>Shorts</div><div onClick={()=>setShowUp(!showUp)} style={{background:'red', width:42, height:42, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, marginTop:-12}}>+</div><div style={{textAlign:'center', color:'#aaa', fontSize:11}}><div style={{fontSize:18}}>📺</div>Subs</div><div style={{textAlign:'center', color:'#aaa', fontSize:11}}><div style={{fontSize:18}}>📚</div>Library</div></div>
    </div>
  )
}
